package
{
	...
	public class DemoVector extends Sprite
	{
		...
		function DemoVector(numQuads:uint, numUploads:uint, bufferUsage:String)
		{
			_numQuads = numQuads;
		...
		private function contextCreateHandler( event:Event ):void
		{
			...
			for (var i:uint = 0; i < _numQuads; i++)
			{
				...
				_vertices.push( pos.x - size, pos.y + size, 0.5 );
				_vertices.push( pos.x + size, pos.y + size, 0.5 );
				_vertices.push( pos.x + size, pos.y - size, 0.5 );
				_vertices.push( pos.x - size, pos.y - size, 0.5 );
		...
		private function enterFrameHandler( event:Event ):void
		{
			...
			{
				_vertexBuffer.uploadFromVector( _vertices, 0, 4 * _numQuads );
